...
import { TerminalCollection, TerminalCollectionWithScripts, ScriptReference, UpdateTerminalCollection, Script, GetTerminalCollections, GetTerminalCollectionById, GetScripts } from "@codestate/core";
import { ConfigurableLogger } from "@codestate/core";

export async function updateTerminalCollectionTui() {
  ...
  try {
    ...
    if (!terminalCollectionsResult.ok || terminalCollectionsResult.value.length === 0) {
      logger.warn("No terminal collections found to update");
      return;
    }

    const terminalCollections = terminalCollectionsResult.value;

    // Step 1: Select terminal collection to update
    ...
    // Get the selected terminal collection details
    const currentCollectionResult = await getTerminalCollectionById.execute(selectedTerminalCollection);
    ...
    // Step 4: Collect updates for each selected field
    ...
    logger.plainLog(`\n💾 Updating terminal collection: ${currentCollection.name}`);
    const result = await updateTerminalCollection.execute(
      selectedTerminalCollection,
      updates
    );

    if (result.ok) {
      logger.log(`Terminal collection '${currentCollection.name}' updated successfully!`);
    } else {
      logger.error(`Failed to update terminal collection: ${result.error?.message}`);
    }

  } catch (error) {
    logger.error("Unexpected error during terminal collection update");
  }
}
